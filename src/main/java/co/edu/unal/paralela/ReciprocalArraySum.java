...
public final class ReciprocalArraySum{
     ...
     * Constructor.
     */
    private double[] arrayElements;
    private ReciprocalArraySum(){
    }

    public double[] getArrayElements() {
        return arrayElements;
    }

    public void setArrayElements(double[] arrayElements) {
    ...
    protected static double parManyTaskArraySum(final double[] input,
        ...
        for (i = 0; i < numTasks - 1; i++) {
            tasks[i] = new ReciprocalArraySumTask(getChunkStartInclusive(i, numTasks, input.length), getChunkEndExclusive(i, numTasks, input.length), input);
            tasks[i].fork();
        }
        tasks[i] = new ReciprocalArraySumTask(getChunkStartInclusive(i, numTasks, input.length), getChunkEndExclusive(i, numTasks, input.length), input);
        tasks[i].compute();
